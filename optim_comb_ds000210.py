...
Optim_comb = Optim_comb[:,:,:,1:]
Optim_vol = Optim_vol[:,:,:,1:]
...
# echo times
echo_times = np.array([13.7,30.,47.])
...
for i in range(0, dim1):
    for j in range(0, dim2):
        for k in range(0, dim3):
            ...
            data_2d = np.concatenate((e1_ts,e2_ts,e3_ts))
            log_data = np.log(np.abs(data_2d) + 1)
            
            x = np.column_stack([np.ones(3), [-te for te in echo_times[:3]]])
            X = np.repeat(x, 203, axis=0)
...
for i in range(0, dim1):
    for j in range(0, dim2):
        for k in range(0, dim3):
            ...
            if t2s_output[i,j,k] == 0:
                Optim_comb[i,j,k,:] = np.zeros((len(e1_ts)))
                Optim_vol[i,j,k,:] = np.zeros((len(e1_ts)))
                continue
...
nib.save(t2s_img, 'sub-30_T2s.nii')

comb_img = nib.Nifti1Image(Optim_comb, echo_1.affine, echo_1.header)
nib.save(comb_img, 'sub-30_Optim_comb.nii')    
